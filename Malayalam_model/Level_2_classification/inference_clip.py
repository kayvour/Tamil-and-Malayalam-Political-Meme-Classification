...
def predict_image(image_path, model_path='best_clip_level2_model.pth'):
    ...
    print(f"Loading CLIP Level 2 model from {model_path}...")
    checkpoint = torch.load(model_path, map_location=device)
    label_map = checkpoint['label_map']
    reverse_map = {v: k for k, v in label_map.items()}
    
    processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
    model = CLIPMemeClassifier(num_classes=len(label_map)).to(device)
    ...
    print("\n" + "="*50)
    print("CLIP LEVEL 2 MODEL PREDICTION")
    print("="*50)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print('Example: python inference_clip.py "path/to/image.jpg"')
        sys.exit(1)
